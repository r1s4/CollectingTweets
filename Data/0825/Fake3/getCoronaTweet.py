...
# 検索内容
keyword = 'ＣＯＶＩＤ－１９はＣｅｒｔｉ…ｉｎｔｅｌｌｉｇｅｎｃｅの略語です。そして１９はこのウイルスが作り出された年を指します。ＣＯＶＩＤ－１９はウイルスの名前ではありません。これは何十年も前から開発されてきた人口抑制と人口削減の為の世界的アジェンダの名前なのです'
''' 
使いそうなやつ　
...
def job():
    ...
    if next_max_id == 0:
        for tweet in api.search(q=keyword, tweet_mode = 'extended', count=100):  #tweet_mode='extended' ツイート本文が途切れないようにする
            try:
                tweet_data.append([tweet.id, tweet.user.screen_name, tweet.created_at, tweet.full_text.replace('\n',''), tweet.favorite_count, tweet.retweet_count, tweet.user.followers_count, tweet.user.friends_count, tweet.user.id])
    ...
    if next_max_id==initial_next_max_id:
        sys.exit()
